...
public class PaymentService {
	 ...
	 * @return
	 */
	private Double checkAvailableSum(Double sum, Account account, Double percent) {
		Double finalSum = null;
		Double balance = account.getBalance();
		Double summa = sum + (sum * Double.valueOf(percent));
	 ...
	 * @param balance
	 */
	private void withdrawFunds(Account account, Double balance) {
	 ...
	 * @param balance
	 */
	private void addFunds(Account account, Double balance) {
		account.setBalance(balance);
		accountDao.update(account);
	}
	 ...
	 */
	public void formPayment(Double sum, String number, Integer accountId, Double percent, String appointment) {
		...
		Double suma = checkAvailableSum(sum, accountFrom, percent);
		if (receiverCard != null && suma != null) {
			Account accountTo = accountDao.findAccountById(receiverCard.getAccountId());
			withdrawFunds(accountFrom, accountFrom.getBalance() - suma);
			addFunds(accountTo, accountTo.getBalance() + sum);
