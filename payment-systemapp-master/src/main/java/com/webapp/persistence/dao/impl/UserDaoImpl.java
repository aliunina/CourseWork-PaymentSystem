...
public class UserDaoImpl implements UserDao {
	...
	private static final String FIND_ALL = "SELECT users.user_id, users.phone, users.password,"
			+ "users.email, users.name, users.surname, users.role_id, roles.role_name FROM users JOIN roles ON users.role_id = roles.role_id and users.role_id=2";
	private static final String FIND_BY_ID = "SELECT users.user_id, users.phone, users.password,"
			+ "users.email, users.name, users.surname, users.role_id, roles.role_name FROM users JOIN roles ON users.role_id = roles.role_id WHERE users.user_id = ?";
	private static final String FIND_BY_LOGIN_PASSWORD = "SELECT users.user_id, users.phone, users.password,"
			+ "users.email, users.name, users.surname, users.role_id, roles.role_name FROM users JOIN roles ON users.role_id = roles.role_id WHERE users.phone = ? AND users.password = ?";
	...
	private static final String FIND_BY_PHONE = "SELECT users.user_id, users.phone, users.password,"
			+ "users.email, users.name, users.surname, users.role_id, roles.role_name FROM users JOIN roles ON users.role_id = roles.role_id WHERE users.phone = ?";
